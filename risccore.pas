...
CONSTRUCTOR riscty.init(filename : string);
...
BEGIN
  ...
  BEGIN
    ...
    IF (ir and qbit) = 0 THEN  c_val:= R[c_] ELSE
                BEGIN
                IF (ir and vbit) = 0 THEN  c_val := im
                                     else  c_val := ($FFFF0000 or im);
                END;
  lastop := op;
  lasttyp := 1;
...
lastb_val := b_val;
lastc_val := c_val;
IF cyclecounter < 3000 THEN coredump;
IF cyclecounter = 3000 THEN coredumpclose;
END; (* proc *)
  ...
  case address-IOStart of
   ...
   12: BEGIN
    ...
    28: BEGIN
       ...
       (*      writeln;
             writeln('scan ', scancodebyte, ' key_cnt ', key_cnt); *)
             dec(key_cnt);
